...
import com.DPN.ProjectS.Config;
import com.DPN.ProjectS.Main;
import com.DPN.ProjectS.ai.EntityAIHuntTinyCreatures;
import com.DPN.ProjectS.ai.EntityAINewOcelotFear;
    ...
    public static void onAddCapabilites(AttachCapabilitiesEvent event) {
        if ((event.getObject() instanceof EntityPlayer || event.getObject() instanceof EntityLivingBase)) {
            ...
            if (entity.isNonBoss() && !entity.hasCapability(SizeProvider.sizeCapability, null)) {
                ...
                event.addCapability(new ResourceLocation(Main.MODID, "size"), new SizeProvider(cap));
            }
        }
    }
