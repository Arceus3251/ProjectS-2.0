...
import com.DPN.ProjectS.util.handlers.EntitySizeHandler;
import com.DPN.ProjectS.util.handlers.RenderEntityHandler;
...
public class EntitySizeUtil {
    ...
    public static double getEntityYOffset(double origOffset, Entity entity) {
        ...
        // return (origOffset) + (((0.2625D * (riddenScale)) - (origOffset)) *
        // (1 - (scale))); // maths
        // return (origOffset) + ((0.5 * entity.height) / scale) - (0.5 *
        // entity.height); // make middle of bounding box constant
        double d = ((0.23125D * riddenScale) - (origOffset));
        return (origOffset) + (((1 / scale) - (1)) * (d * (entity.height / 2)))
                + (((riddenScale - 1) * (0.225D)) * scale);
    }
    ...
    @SideOnly(Side.CLIENT)
    public static void doRenderEntity(Entity entityIn, int x, int y, int z, float yaw, float partialTicks,
                                      boolean p_188391_10_) {
        RenderManager rendermanager = Minecraft.getMinecraft().getRenderManager();

        float entityScale = EntitySizeUtil.getEntityScale(entityIn);
        float zoom = 1 / entityScale;
        String scaleText = "x" + entityScale;
    ...
    public static boolean isOnLadder(Entity entity) {
        if (entity instanceof EntityPlayer && getEntityScale(entity) <= TINY_THRESHOLD && entity.collidedHorizontally) {
            ...
            return (player.getHeldItemMainhand().getItem() == Items.SLIME_BALL
                    && player.getHeldItemOffhand().getItem() == Items.SLIME_BALL);
        }
        return false;
    }
